A prompt is a briefing, not a spell. Paste one, take whatever comes back and put your name on it, and this will go badly. Every prompt below assumes you know things the model does not: what your entity does, which numbers matter this period, what your reviewer will ask, and what happens if the answer is wrong. The prompt's job is to get that out of your head and in front of the model in an orderly way. Yours is everything after.

Three things are worth changing in almost every prompt here. The role line, because "you are assisting a finance team in a group with several overseas subsidiaries and a manual consolidation" produces different work from "you are assisting a finance team". The constraints, so they carry your house rules, your thresholds, your conventions — anything a new joiner would get wrong. And the output format, so it matches the document the output actually feeds, and you are pasting into a template rather than reformatting by hand.

Two things are worth leaving alone: the instruction to mark uncertainty and refuse to guess, and the ban on inventing figures. Remove either and you get confident filler, which is the expensive failure mode in this work. Before any of it, settle a separate question — whether you are allowed to put this particular information into this particular tool. Client data, staff data, unpublished results and anything under a confidentiality obligation are decisions for whoever owns that policy, not for you at your desk on a Tuesday. If nobody has decided, assume no.
...
| The output format, to fit your template | The instruction to list what it could not determine |

## 1. Month-end close and reconciliations

**P1.1 — Reconciliation break triage**
*Use when:* you have a list of unexplained differences and little time.
*Paste this:*
```
You are assisting a finance team at month-end. I will paste the
reconciling items on [ACCOUNT] for [PERIOD].

Task: group them by likely underlying cause, and for each group state
what evidence would confirm or rule that cause out.

Constraints: use only what I paste. Invent no amount, date or
counterparty. Where an item could belong to two groups, say so rather
than choosing.

Format: a table — Item, Amount as given, Likely cause, Confirming
evidence, Who to ask. Below it, a list headed "Cannot classify from
this information", with a reason for each.
...
[PASTE]
```
*Check before you use the output:* the groupings are inference, not findings. Watch for items assigned a cause because they resembled the one above.

**P1.2 — Break explanation, written up**
*Use when:* you know why the difference arose and must write it for the file.
*Paste this:*
```
...
Constraints: use only the facts in my notes. Add no causes, amounts,
dates or conclusions. Do not soften or strengthen my conclusion. Plain
British English, no reassuring adjectives.

Format: four short paragraphs — what the difference was, why it arose,
what was done, what remains open. Under 300 words.

Do not fill gaps. List them under "Questions I could not answer from
your notes".

Notes:
[PASTE]
```
*Check before you use the output:* the "what remains open" paragraph. Models tidy loose ends into resolutions; if you left something unresolved, the note must still say so.

**P1.3 — Close checklist builder**
*Use when:* your close runs on memory and one person's spreadsheet.
*Paste this:*
```
You are helping a finance team document its month-end close. I will
describe our tasks in my own words, roughly in order. The close runs
from working day [X] to working day [Y].

Task: turn this into a checklist someone who has never run our close
could follow.

Constraints: include only tasks I describe. Do not add standard tasks
you assume we perform, and do not assign deadlines I have not given.
...
Where my description is ambiguous about sequence or ownership, write
UNCLEAR rather than choosing.

Description:
[PASTE]
```
*Check before you use the output:* the dependency column. A checklist that puts a task before what it depends on is worse than none, and that field gets filled by pattern.

**P1.4 — Intercompany mismatch plan**
*Use when:* two entities disagree and you need a plan before the calls start.
*Paste this:*
```
You are assisting a group finance team. [ENTITY A] and [ENTITY B]
disagree on the intercompany balance for [PERIOD]. I will describe what
each reports and what I already know.

Task: produce an investigation plan — causes to test in order, cheapest
and most likely first.

Constraints: work only from what I tell you. Assume nothing about our
systems, currencies, cut-off conventions or intercompany arrangements.
Calculate no difference I have not given you.

Format: numbered steps. Each states what to check, what result confirms
the cause, and what to do if it does not.

End with "Assumptions I had to make", listing anything you treated as
given that I did not state. If you need information I have not
provided, ask for it instead of proceeding.
...
[PERIOD], which I will paste as text.

Task: produce the questions a careful reviewer would ask — aimed at what
would be wrong if the preparer had been rushing.

Constraints: recalculate nothing and call no figure wrong; you cannot
see the evidence. Ask questions instead. Name no accounting standard;
where a point turns on recognition, describe it plainly and mark it
"judgement — confirm against your own reference sources".

Format: questions grouped under Completeness, Cut-off, Basis of
estimate, and Reversal. Maximum 20, ordered by how much money turns on
them.

Where the schedule gives you too little to form a real question, say so
rather than asking a generic one.

Schedule:
[PASTE]
```
*Check before you use the output:* whether the questions are specific to your schedule. Generic ones mean it lacked context, and they will waste your preparer's afternoon.
...
[ENTITY], [PERIOD]. I will paste a table of actual, comparative and
difference.

Task: for each line I flag, list the questions to put to the budget
holder — the ones distinguishing a timing difference, a volume
difference, a price or rate difference, a misposting, and a real change
in the business.

Constraints: do not explain the variances yourself; you do not know our
business. Calculate nothing and restate no figure. Do not let a
presumed cause appear in the wording of a question.
...
[PASTE]
```
*Check before you use the output:* leading questions. "Why did costs rise due to the new hires?" gets you the answer you suggested, not the true one.
...
[AUDIENCE]. For each variance I will give the account, the figure, and
the explanation obtained from the business. That is the only permitted
source of cause.

Task: write the commentary.

Constraints: every cause must come from my notes. Add no context,
market conditions, trends or forward-looking statements. Add no figure
I have not given. Do not use "significant", "material", "strong" or
"challenging". Do not tell the reader how to feel. British English.

Format: one short paragraph per variance, largest first, each opening
with the account name in bold. Maximum [WORD LIMIT] words.

Where an explanation does not actually account for the movement, flag
it at the end rather than writing around it.

Notes:
[PASTE]
```
*Check before you use the output:* every causal phrase — "because", "driven by", "reflecting". Each must trace to something a named person actually told you.
...
[CURRENT PERIOD] at [ENTITY]. I will give the opening figure, the
closing figure, and the components with amounts and my explanation of
each.
...
do not, stop and tell me the difference — do not adjust anything or
insert a balancing item. Add no components and no causes of your own.

Format: a lead sentence stating the movement, one sentence per
component, and a closing sentence on the next period only if I have
...
[PASTE]
```
*Check before you use the output:* whether it reported a reconciliation failure. Smooth prose over components that do not add up hides your error rather than yours being found.
...
Constraints: change no figure, no cause and no conclusion. Add no
facts. If cutting a sentence would lose a fact, keep the fact and cut
the decoration. British English. Preserve anything marked [DO NOT
CHANGE].

Format: the rewritten text, then a list of sentences you could not
tighten because they contained a claim you could not verify from the
draft itself.

Never invent a stronger version of a weak claim. If a sentence asserts
...
will give my explanation for a movement in [MEASURE] at [ENTITY] for
[PERIOD].
...
Constraints: do not present alternative explanations as fact, and
invent no figures to support a challenge. Frame every challenge as a
question I can go and answer. Assume I am competent and sceptical of
you too.

Format: challenges in three groups — does the cause explain the size,
does it explain the timing, is there a simpler explanation. Maximum
ten, hardest first.

If you cannot fault it on the information given, say so plainly rather
than manufacturing objections.

My explanation:
[PASTE]
```
*Check before you use the output:* whether each challenge is answerable with evidence you can actually obtain. Park the rest until after the meeting.
...
*Paste this:*
```
You are helping a finance team make its assumptions explicit for
...
one number. You do not know our sector or our size. Where a driver
depends on a decision rather than a fact, mark it "decision required"
and say whose.

Format: a table — Driver, Unit, Type (fact / estimate / decision),
Where the answer lives, Question to ask.

If the line could sensibly be built more than one way, show the two
commonest structures and what would make one preferable.
```
*Check before you use the output:* that the drivers actually add or multiply back to your line. A tidy list that does not reconstruct the number is decoration.
...
submit figures for [PERIOD]. Their cost lines are [LIST]. The deadline
is [DATE]. Last cycle's problems were [LIST].

Task: write the briefing note that accompanies the template.

Constraints: no finance jargon without a plain gloss on first use. No
threats and no cheerleading. State no figure, limit or target — I will
insert those. Promise nothing about what finance will do with the
numbers beyond what I have told you.
...
Leave a marked [ ] placeholder wherever a figure, limit or name is
needed rather than supplying one.
```
...
which are inconsistent with each other, and which are stated too
vaguely to be tested later.

Constraints: do not tell me an assumption is too high or too low — you
have no basis for that. Restrict yourself to sensitivity, internal
consistency and testability. Compute no revised outcomes.

Format: three sections — Most sensitive, Mutually inconsistent, Not
testable as written. Name the assumption and state precisely what the
problem is.

Where two assumptions look inconsistent but might be reconciled by
something I have not told you, phrase it as a question.

Assumptions:
[PASTE]
```
*Check before you use the output:* the sensitivity claims. Without your model it reasons from structure alone, so treat them as a list to test, not a finding.
...
state which of my named uncertainties resolve which way, and stay
internally consistent — if demand falls, do not also assume prices hold
unless you say why. Use only the uncertainties I give you.

Format: for each — a name, a one-paragraph description, bullets showing
which assumptions move and in which direction, and the earliest
observable sign that this scenario is happening.

If two of my uncertainties cannot vary independently, say so.
...
Task: check the submission against the rules only. List every breach,
and every required explanation that is missing or too vague to be one.

Constraints: do not assess whether the figures are reasonable — that is
mine to judge. Do not compare to prior year unless I give you prior
year. Quote the submission's exact wording whenever you flag something.

Format: a table — Rule, What the submission says, Problem, What to
request. Then one line: ready for review, or return to sender.

If a rule is ambiguous as written, flag the rule rather than choosing a
reading.
...
Task: for each item, say in plain words what is likely being
established, what document or extract would satisfy it, and which of my
team roles normally holds it.
...
state that at the top. Cite no standard, regulation or pronouncement.
...
Where you cannot tell what is being asked for, write "ask the audit
team to clarify".

List:
[PASTE]
```
*Check before you use the output:* the "likely purpose" column. It is inference, and sending documents on a wrong inference means doing the work twice.
...
[TOPIC] for [ENTITY], [PERIOD]. I will give the query and the facts and
documents we hold.

Task: draft the response.

Constraints: use only the facts I supply. Do not assert that a
treatment is correct, compliant or in accordance with anything — state
what was done and why, and let the auditor form their view. Name no
standard, regulation or authority. Commit us to nothing I have not
said. Neutral tone, no defensiveness.

Format: our position in short; the facts in order; the documents
attached; any point we accept remains a matter of judgement.

If my facts do not fully answer the query, say what is missing rather
...
Constraints: describe only what I say happens, weaknesses included. Do
not write the control as it ought to be. Add no frequency, threshold,
approver or evidence I have not stated. Name no control framework.
Never use the word "ensures" — say what the control does and what it
would catch.

Format: What it is; Who performs it; When and how often; What evidence
it leaves; What it would catch; What it would not catch.

The last section is the point of the exercise. If my description does
not let you complete it honestly, say so and ask me the questions that
would.
...
presume the process is being followed. Assess nothing — you are writing
questions, not conclusions. No framework or standard references.
...
the manual rather than their day" — the ones that surface workarounds.
...
[PERIOD]. I will paste what we intend to test, how, and over what
population.

Task: find where the evidence obtained would not support the conclusion
drawn — gaps between what a test proves and what the approach claims it
proves.

Constraints: this is a logical review, not a professional or regulatory
one; state that at the top. Do not say whether the approach meets any
standard, and name none. Propose no sample sizes or statistical
parameters. Assume nothing about our population.
...
[PASTE]
```
*Check before you use the output:* whether it stayed inside logic. The moment it starts telling you what is required, it has left the ground it can be trusted on.

## 5. Tax and compliance preparation

Everything in this group is preparation, not advice, and no output here should reach a filing without a qualified person in your jurisdiction reviewing it. Tax rules differ between countries, differ within them, and change — often with effect from a date that has already passed. Use these to get organised, not to reach conclusions.
...
Task: produce a checklist of documents and records to gather for
whoever prepares the filing.
...
adviser will review this. Never state what is deductible, taxable,
claimable or reportable. Reference no tax law, section, regulator or
form. Assume no jurisdiction.

Format: a table — Activity, Records to gather, Where they usually live,
Owner, Status.

Where treatment plainly depends on local rules, write
"jurisdiction-dependent — for your adviser" instead of a conclusion.
...
it goes to our tax adviser. I will paste it with descriptions and
amounts.

Task: identify the transactions whose description is too vague, generic
or inconsistent for someone else to categorise, and write the question
to put to whoever incurred them.
...
[PASTE]
```
*Check before you use the output:* whether purpose has been inferred from vendor names. "Software supplier, therefore software cost" is the assumption that needs unwinding later.

**P5.3 — Understanding a letter you have received**
*Use when:* correspondence arrives and you need to know what is being asked.
*Paste this:*
```
You are helping a finance professional understand correspondence. I
will paste the text of a letter received by [ENTITY].
...
or obligations are. Name no law, regulator or authority. Work only from
...
about [TOPIC] at [ENTITY]. I will describe our situation and what we
already know.

Task: write the questions to ask, ordered so the answers build on each
other, and note what documents to have to hand for each.
...
Format: numbered questions, each with a sub-line "Bring:" listing
documents.
...
Constraints: use only the obligations and dates I give. Add no
obligations you assume apply to us, and correct none of my dates — you
do not know my jurisdiction and requirements change. Anything you think
I may have omitted goes in a separate list, phrased as questions.

Format: a table — External deadline, Obligation, Internal milestone,
Days before, Owner. Sorted by date.

Open with a line stating that the dates come from me and must be
...
Reading a contract for its finance consequences is not the same as advising on it. These prompts extract and organise. They do not interpret, and none of them replaces your legal adviser.

**P6.1 — Finance summary of an agreement**
*Use when:* a signed agreement lands and you need to know what it does to your numbers.
*Paste this:*
```
You are helping a finance team understand an agreement's financial
mechanics. I will paste the text of an agreement involving [ENTITY].
...
jurisdiction and a qualified adviser will review the legal questions.
Quote the clause reference and the operative wording for every point.
Do not interpret ambiguous drafting — flag it. State no accounting
treatment. Summarise no clause you were not given.

Format: sections for Amounts payable or receivable; Timing and
triggers; Variation and escalation; Termination and its financial
consequences; Ongoing obligations to track.
...
Task: extract the payment terms into a structured table for system
entry.
...
consequence, price adjustment mechanism, discount or rebate terms,
expenses, disputed amounts.

Below it, list any payment-related provision that did not fit these
fields.

Contract:
[PASTE]
```
*Check before you use the output:* every "not stated", and any field with an odd clause reference. Extraction failures cluster in documents with unusual numbering.
...
Law differs by jurisdiction and changes. Confine yourself to: this is
what you said the deal is, this is what the words say, here is the
question.
...
Where drafting matches my description, say so rather than inventing a
concern.
...
give the rules we have decided, the limits we have set, and the
behaviour we are trying to change.

Task: write the policy.

Constraints: include only rules I have given. Add no limits,
thresholds, approval levels or exceptions of your own — if one seems
missing, list it as a question. Reference no law or regulation; where a
rule exists because of an external requirement, I will add that. Write
for someone reading it once, in a hurry. British English. No sentence
over 25 words.
...
*Paste this:*
```
You are structuring a board paper on [TOPIC] for [ENTITY]. I will tell
you the decision or noting item required, what the board already knows,
and the facts I hold.
...
End with "Facts you will need that you have not mentioned" — the gaps a
board would notice, as questions, not assumptions.
...
audience is [AUDIENCE]; the decision required is [DECISION].
...
what the reader must know to make it; the main risk or uncertainty;
what happens next.
...
likely feelings. British English.

Format: subject line, then the message, under 200 words.

Then give one alternative opening paragraph with a different balance of
directness, and one line on when each is the better choice.
...
but does not answer, questions it does not anticipate at all. Hardest
first within each.
...
and add a short gloss in brackets on first use — do not swap in a loose
synonym. Never simplify a conditional statement into an unconditional
one. Add no examples, analogies or context of your own. British
English.
...
leave it and list it under "Kept as written".
...
source gives detail; a "Before you start" list.

End with "Points the description left open", plus a note wherever you
changed the order.
...
[ENTITY], covering [TASK]. I will paste our existing procedure and say
what the joiner does and does not already know.
...
each; then "When to stop and ask", listing the situations where a
newcomer should escalate rather than proceed.
...
[PASTE]
```
*Check before you use the output:* the "when to stop and ask" section, with the person who currently does the job. Only they can write it, and it is what prevents expensive mistakes.
...
benchmark data. Reference no real organisation. Each exercise needs a
definite right answer, or — if it is a judgement exercise — a definite
...
status; Who to contact for what; Access and permissions required; Known
problems and workarounds; Things only this person knows.

The last section matters most. Prompt me with questions to fill it —
the undocumented knowledge that walks out of the door.
...
*Paste this:*
```
You are documenting a spreadsheet formula for a future maintainer. I
will paste the formula and describe the sheet layout and what the
calculation is meant to achieve.
...
Assume no ranges, named ranges or sheet contents I have not given you.
Do not rewrite the formula unless asked. Where an edge case cannot be
...
End with "Where this may not match your stated intent", or say plainly
