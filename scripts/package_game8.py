#!/usr/bin/env python3
"""Package Critter Beat (the rhythm-tap game).

- site/play8/                       free web version (Cloudflare beacon
  injected — site pages only, per fleet rules)
- dist/Critter-Beat-playables.zip   Playgama submission build: bundles
  the Playgama Bridge SDK (game8/vendor/, shared) + config, sends
  game_ready on init. index.html at zip root, NO analytics.
- dist/Critter-Beat-itch.zip        itch.io build, vanilla (no SDK),
  no external calls

Free by design: discovery game feeding the Wildhaven world.

    python3 scripts/package_game8.py
"""
...
GAME = os.path.join(ROOT, "game8")
VENDOR = os.path.join(ROOT, "game6", "vendor")   # one shared SDK copy
PLAY = os.path.join(ROOT, "site", "play8")
...
def main():
    ...
    for zname, zhtml, with_sdk in (
            ("Critter-Beat-playables.zip", playgama_html, True),
            ("Critter-Beat-itch.zip", html, False)):
    ...
    print("site: site/play8/ (beacon on)")
