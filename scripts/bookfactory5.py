#!/usr/bin/env python3
"""MATCHA book factory (bookfactory5).

    python3 scripts/bookfactory5.py status   # progress + next chapter brief
    python3 scripts/bookfactory5.py build    # EPUB + cover into dist/

Chapters live in bookfactory5/chapters/<id>.md; the plan (7 parts, 40
chapters) in bookfactory5/plan.json; art in bookfactory5/art/<id>.png
(regenerate with scripts/matcha_art.py).
...
h3 { font-size: 1.05em; margin: 1.2em 0 0.3em; color: #3E5A3A; }
p { margin: 0 0 0.85em; text-indent: 0; }
ul { margin: 0 0 0.9em 1.2em; }
li { margin-bottom: 0.35em; }
table { border-collapse: collapse; margin: 0 0 1em; width: 100%; }
th, td { border: 1px solid #A8C686; padding: 0.35em 0.5em; text-align: left; font-size: 0.92em; }
...
.part .k { font-size: 0.9em; letter-spacing: 0.3em; color: #7BA05B; }
...
def build():
    ...
    cover_path = os.path.join(DIST, "Matcha-cover.jpg")
    if not os.path.exists(cover_path):
        sys.path.insert(0, os.path.join(ROOT, "scripts"))
        import matcha_art
        matcha_art.make_cover(cover_path)

    suffix = "" if complete else "-PREVIEW-%dof%d" % (len(done), len(chs))
    epub_path = os.path.join(DIST, "Matcha%s.epub" % suffix)

    uid = "urn:uuid:%08x-mat5-4000-8000-%012x" % (abs(hash(plan["title"])) % 2**32,
    ...
    about = ("<h1>About this book</h1>"
             "<p>This book shares the history, craft, and kitchen life of matcha, "
             "with recipes the author trusts. Where it touches on health, it offers "
             "general information only — it is not medical or nutritional advice. "
             "Recipes are starting points: ingredients, ovens, and tastes vary, so "
             "taste as you go and make each one yours.</p>"
    ...
    _also = also_by_html(skip="matcha")
    if _also:
...
<spine>%s</spine>
    ...
    if complete:
        print("COMPLETE — ready for KDP upload (EPUB + dist/Matcha-cover.jpg).")
    return complete
...
def status():
    ...
    for _, ch in done:
        ...
        if os.path.exists(p):
            with open(p) as f:
                words += word_count(f.read())
    print("%s — %d/%d chapters (~%d words of target ~140k)"
    ...
    if todo:
        ...
        print("\nAll chapters written. Run: python3 scripts/bookfactory5.py build")
