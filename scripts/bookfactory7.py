...
Chapters live in bookfactory7/chapters/<id>.md; the plan (8 parts, 68
chapters) in bookfactory7/plan.json; art in bookfactory7/art/<id>.png
(regenerate with scripts/ai_art.py).
...
def status():
    ...
    for _, ch in done:
        ...
        if os.path.exists(p):
            with open(p) as f:
                words += word_count(f.read())
    print("%s — %d/%d chapters (~%d words of target ~210k)"
    ...
    if todo:
        ...
        print("\nAll chapters written. Run: python3 scripts/bookfactory7.py build")
