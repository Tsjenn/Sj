#!/usr/bin/env python3
"""The Amah's Daughter novel factory (bookfactory6).

    python3 scripts/bookfactory6.py status   # progress + next chapter brief
    python3 scripts/bookfactory6.py build    # EPUB + cover into dist/

Chapters live in bookfactory6/chapters/<id>.md; the plan (5 parts, 36
chapters) in bookfactory6/plan.json; art in bookfactory6/art/<id>.png
(regenerate with scripts/saga_art.py).
...
h3 { font-size: 1.05em; margin: 1.2em 0 0.3em; color: #1F3A5F; }
p { margin: 0 0 0.85em; text-indent: 0; }
ul { margin: 0 0 0.9em 1.2em; }
li { margin-bottom: 0.35em; }
table { border-collapse: collapse; margin: 0 0 1em; width: 100%; }
th, td { border: 1px solid #C9A84C; padding: 0.35em 0.5em; text-align: left; font-size: 0.92em; }
th { background: #EFF3E4; }
...
.part .k { font-size: 0.9em; letter-spacing: 0.3em; color: #A64B2A; }
...
def build():
    ...
    cover_path = os.path.join(DIST, "AmahsDaughter-cover.jpg")
    if not os.path.exists(cover_path):
        sys.path.insert(0, os.path.join(ROOT, "scripts"))
        import saga_art
        saga_art.make_cover(cover_path)

    suffix = "" if complete else "-PREVIEW-%dof%d" % (len(done), len(chs))
    epub_path = os.path.join(DIST, "The-Amahs-Daughter%s.epub" % suffix)

    uid = "urn:uuid:%08x-ama6-4000-8000-%012x" % (abs(hash(plan["title"])) % 2**32,
    ...
    about = ("<h1>Author's Note</h1>"
             "<p>This is a work of fiction. The Tan household, its members, and "
             "their fates are invented, and no character is drawn from any real "
             "private person. The world around them, however, is rendered as "
             "faithfully as care allows: Malacca in the 1930s, the invasion of "
             "Malaya in December 1941, the occupation years and their screenings "
             "— remembered here with gravity and without invented particulars — "
             "the Emergency, and merdeka in 1957. Where history and story meet, "
             "history was given the right of way.</p>"
             "<p>The black-and-white amahs — the majie — were real, as were the "
             "nyonya beaders whose kasut manek carried brides across thresholds "
             "for generations. This book is offered in admiration of both.</p>"
             "<p>All illustrations are original artwork made for this book.</p>")
...
<spine>%s</spine>
    ...
    if complete:
        print("COMPLETE — ready for KDP upload (EPUB + dist/AmahsDaughter-cover.jpg).")
    return complete
...
def status():
    ...
    for _, ch in done:
        ...
        if os.path.exists(p):
            with open(p) as f:
                words += word_count(f.read())
    print("%s — %d/%d chapters (~%d words of target ~150k)"
    ...
    if todo:
        ...
        print("\nAll chapters written. Run: python3 scripts/bookfactory6.py build")
